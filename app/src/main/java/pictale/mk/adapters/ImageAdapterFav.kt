...
import pictale.mk.ImageActivityFav
import pictale.mk.R


class ImageAdapterFav(val context: Context,
                      private val items: List<Uri>,
                      val eventId: String?,
                      val name: String?,
                      val location: String?) :
    RecyclerView.Adapter<ImageAdapterFav.ViewHolder>() {
    val data=items
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.picture.setOnClickListener {
            d("click-pic","$id")
            val intent = Intent(context, ImageActivityFav::class.java)
            val uri: Uri = id
